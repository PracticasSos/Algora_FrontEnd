...
const AbonosSection = ({ branchId, formData, setAbonosTotals }) => {
    ...
    useEffect(() => {
        const fetchDailyWithdrawals = async () => {
            ...
            // Get sales for the branch and not refund
            let salesQuery = supabase
                .from("sales")
                .select(`id, branchs_id, patients(pt_firstname, pt_lastname), total, payment_balance, credit, is_refund`)
            ...
            let withdrawalsQuery = supabase
                .from("withdrawals")
                .select(`id, sale_id, previous_balance, new_balance, difference, date, sales(branchs_id, payment_balance, total, credit, patients(pt_firstname, pt_lastname))`)
                .in("sale_id", saleIds)
                .eq("sales.branchs_id", Number(branchId));
            ...
            if (withdrawalsError || !withdrawalsToday) {
                setWithdrawalsRecords([]);
                setAbonosTotals({ EFEC: 0, TRANS: 0, DATAF: 0, total: 0 });
                setTotalAbonosDelDia({ EFEC: 0, TRANS: 0, DATAF: 0, abonosDelDia: 0 });
                return;
