...
const Branch = () => {
  ...
  const fetchBranches = async () => {
    setLoading(true);
    const { data, error } = await supabase.from("branchs").select("*").order("name", { ascending: true });
    if (error) {
      toast({ title: "Error al cargar sucursales", description: error.message, status: "error", duration: 5000, isClosable: true, position: "top" });
    } else {
      setBranches(data || []);
    }
    setLoading(false);
  ...
  const filteredBranches = useMemo(() => {
    ...
    return branches.filter(matches);
  }, [branches, search]);

  const activeBranches = useMemo(() => filteredBranches.filter((l) => l.active !== false), [filteredBranches]);
  const inactiveBranches = useMemo(() => filteredBranches.filter((l) => l.active === false), [filteredBranches]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (editingId) {
      const { error } = await supabase.from("branchs").update(formData).eq("id", editingId);
      setSaving(false);
      if (error) {
        toast({ title: "Error al actualizar", description: error.message, status: "error", duration: 5000, isClosable: true, position: "top" });
        return;
      }
      toast({ title: "Sucursal actualizada", status: "success", duration: 3500, isClosable: true, position: "top" });
    } else {
      const { error } = await supabase.from("branchs").insert([formData]);
      setSaving(false);
      if (error) {
        toast({ title: "Error al registrar", description: error.message, status: "error", duration: 5000, isClosable: true, position: "top" });
        return;
      }
      toast({ title: "Sucursal registrada con éxito", status: "success", duration: 3500, isClosable: true, position: "top" });
    }
  ...
  const handleReactivate = async (id) => {
    const { error } = await supabase.from("branchs").update({ active: true }).eq("id", id);
    if (error) {
      toast({ title: "Error al reactivar", description: error.message, status: "error", duration: 5000, isClosable: true, position: "top" });
      return;
    }
    toast({ title: "Sucursal reactivada", status: "success", duration: 3000, isClosable: true, position: "top" });
    fetchBranches();
  };
  ...
  const handleConfirmedAction = async () => {
    ...
    if (mode === "deactivate") {
      const { error } = await supabase.from("branchs").update({ active: false }).eq("id", id);
      onDeleteClose();
      ...
      toast({ title: "Sucursal desactivada", description: "Ya no aparecerá en el listado principal. Puedes reactivarla cuando quieras.", status: "info", duration: 4500, isClosable: true, position: "top" });
      fetchBranches();
      return;
    }

    // mode === "delete": borrado definitivo, solo debería usarse en sucursales
    ...
    if (error) {
      ...
      toast({
        ...
        description: isReferenced
          ? "Esta sucursal tiene ventas, gastos u otros registros asociados. Déjala desactivada en vez de eliminarlo — así conservas ese historial."
          : error.message,
        status: "warning",
        duration: 6000,
        isClosable: true,
    ...
    toast({ title: "Sucursal eliminada definitivamente", status: "info", duration: 3000, isClosable: true, position: "top" });
    fetchBranches();
  };
      ...
      <Box maxW="1100px" mx="auto" px={{ base: 4, md: 6 }} pt={8}>
        ...
        <Flex justify="space-between" align="center" mb={6} flexWrap="wrap" gap={3}>
          <HStack spacing={3}>
            ...
            >
              <Icon as={Building2} boxSize="20px" />
            </Flex>
            <VStack align="start" spacing={0}>
              ...
              <Text fontSize="xs" color={subtitleColor}>
                Registra y administra las sucursales de Veoptics
              </Text>
            </VStack>
          </HStack>
          <Button
        ...
        <Flex mb={5} gap={3} align="center" flexWrap="wrap">
          ...
          <Badge
            ...
            fontWeight="bold"
          >
            {activeBranches.length} {activeBranches.length === 1 ? "activo" : "activos"}
          </Badge>
          {inactiveBranches.length > 0 && (
            <Button
              ...
              borderRadius="10px"
            >
              {showInactive ? "Ocultar" : "Ver"} inactivos ({inactiveBranches.length})
            </Button>
          )}
        </Flex>
        ...
        {loading ? (
          ...
          >
            <Icon as={Building2} boxSize="32px" color={subtitleColor} mb={3} />
            <Text color={subtitleColor} fontWeight="semibold">
              {search ? "No se encontraron sucursales con ese criterio" : "Todavía no has registrado ninguna sucursal"}
            </Text>
            {!search && (
        ...
        {showInactive && inactiveBranches.length > 0 && (
          <Box mt={8}>
            <SectionTitle icon={Archive} sectionIconBg={sectionIconBg}>Sucursales inactivas</SectionTitle>
            <SimpleGrid columns={{ base: 1, md: 2, lg: 3 }} spacing={5}>
              {inactiveBranches.map((lab) => (
                <Box
                  key={lab.id}
                  bg={cardBg}
                  borderRadius="24px"
      ...
      <Modal isOpen={isOpen} onClose={onClose} isCentered size="lg">
        ...
        <ModalContent borderRadius="24px" overflow="hidden" mx={4}>
          ...
          <form onSubmit={handleSubmit}>
            <ModalBody pb={2}>
              <SectionTitle icon={Building2} sectionIconBg={sectionIconBg}>Datos generales</SectionTitle>
              <SimpleGrid columns={{ base: 1, md: 2 }} spacing={4}>
                <FormControl isRequired>
                  <FormLabel fontWeight="semibold" fontSize="sm" color={subtitleColor}>Nombre</FormLabel>
                  <Input name="name" value={formData.name} onChange={handleChange} placeholder="Ej: Sucursal Centro" bg={inputBg} borderColor={borderColor} borderRadius="10px" _focus={{ borderColor: ACCENT, boxShadow: `0 0 0 1px ${ACCENT}` }} />
                </FormControl>
                <FormControl isRequired>
                ...
                <FormControl isRequired>
                  ...
                  <Input type="email" name="email" value={formData.email} onChange={handleChange} placeholder="sucursal@email.com" bg={inputBg} borderColor={borderColor} borderRadius="10px" _focus={{ borderColor: ACCENT, boxShadow: `0 0 0 1px ${ACCENT}` }} />
                </FormControl>
                <FormControl isRequired>
        ...
        <AlertDialogOverlay>
          <AlertDialogContent borderRadius="20px" mx={4}>
            <AlertDialogHeader fontWeight="800">
              {pendingAction?.mode === "delete" ? "Eliminar definitivamente" : "Desactivar sucursal"}
            </AlertDialogHeader>
            <AlertDialogBody>
              {pendingAction?.mode === "delete"
                ? "Esta acción no se puede deshacer. Si la sucursal tiene ventas, gastos u otros registros asociados, el sistema no permitirá eliminarlo."
                : "La sucursal dejará de aparecer en el listado principal, pero conserva todo su historial. Puedes reactivarlo cuando quieras."}
            </AlertDialogBody>
